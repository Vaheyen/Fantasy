const budget = 1000;
  ...
  {id:191,name:"Luis Suárez",team:"Sporting",pos:"FWD",price:9}
    ...
    if (leagueTeams.length) viewedLeagueTeamId = leagueTeams[0].id;
    currentUser = loadSessionUser();
    if (currentUser) draftTeamName = currentUser.teamName;
    ...
    function renderCreation() {
      const isTransferMode = teamName.trim() !== "";
      const title = isTransferMode ? "Transfer Mode" : "Create Team";
      const buttonLabel = isTransferMode ? "Update Team" : "Save Team";
      const app = document.getElementById("app");
      const filteredPlayers = getFilteredPlayers();
      const teamOptions = getCreationTeamOptions();

      app.innerHTML = `
        <div class="top-bar">
          <h2 style="margin:0;">${title}</h2>
          ${currentUser ? `<div style="color:#bfdbfe;font-size:13px;">${currentUser.email}</div>` : ""}
          ${isTransferMode ? `<button class="secondary-btn" onclick="renderMain()">Back to Team</button>` : ""}
          ${currentUser ? `<button class="danger-btn" onclick="logoutUser()">Log Out</button>` : ""}
        </div>
        ...
        <div class="top-bar">
          <input
            ...
            value="${draftTeamName.replace(/"/g, "&quot;")}"
            oninput="handleTeamNameInput(this.value)"
          />
          <button id="saveTeamBtn" onclick="saveTeam()" ${canSaveTeam() ? "" : "disabled"}>${buttonLabel}</button>
        </div>

        <div class="summary tight-summary">
          <div class="summary-box glass"><strong>Budget</strong><span>${budget - spentBudget()}M</span></div>
          <div class="summary-box glass"><strong>Picked</strong><span>${selected.length}/15</span></div>
          <div class="summary-box glass"><strong>GK</strong><span>${countPos("GK")}/2</span></div>
          <div class="summary-box glass"><strong>DEF</strong><span>${countPos("DEF")}/5</span></div>
          <div class="summary-box glass"><strong>MID</strong><span>${countPos("MID")}/5</span></div>
          <div class="summary-box glass"><strong>FWD</strong><span>${countPos("FWD")}/3</span></div>
        </div>
        ...
        <div class="layout">
          <div class="panel glass">
            <div class="panel-header">
              <div>
                <h3 style="margin-bottom:4px;">Choose Players</h3>
                <div style="color:#bfdbfe;font-size:13px;">Filter by position, team, and price. Click a selected player again to remove him.</div>
              </div>
            </div>
            ...
            <div class="filter-wrap">
              <div class="filter-grid">
                <div class="filter-box">
                  <label class="filter-label">Filter Position</label>
                  <select onchange="setCreationPositionFilter(this.value)">
                    <option value="ALL" ${creationPositionFilter === "ALL" ? "selected" : ""}>All Positions</option>
                    <option value="GK" ${creationPositionFilter === "GK" ? "selected" : ""}>GK</option>
                    <option value="DEF" ${creationPositionFilter === "DEF" ? "selected" : ""}>DEF</option>
                    <option value="MID" ${creationPositionFilter === "MID" ? "selected" : ""}>MID</option>
                    <option value="FWD" ${creationPositionFilter === "FWD" ? "selected" : ""}>FWD</option>
                  </select>
                </div>
                <div class="filter-box">
                  <label class="filter-label">Filter Team</label>
                  <select onchange="setCreationTeamFilter(this.value)">
                    <option value="ALL" ${creationTeamFilter === "ALL" ? "selected" : ""}>All Teams</option>
                    ${teamOptions.map(team => `<option value="${team}" ${creationTeamFilter === team ? "selected" : ""}>${team}</option>`).join("")}
                  </select>
                </div>
                <div class="filter-box">
                  <label class="filter-label">Sort Players</label>
                  <select onchange="setCreationSortBy(this.value)">
                    <option value="DEFAULT" ${creationSortBy === "DEFAULT" ? "selected" : ""}>Default</option>
                    <option value="POSITION" ${creationSortBy === "POSITION" ? "selected" : ""}>By Position</option>
                    <option value="TEAM" ${creationSortBy === "TEAM" ? "selected" : ""}>By Team</option>
                    <option value="PRICE_ASC" ${creationSortBy === "PRICE_ASC" ? "selected" : ""}>Price Low → High</option>
                    <option value="PRICE_DESC" ${creationSortBy === "PRICE_DESC" ? "selected" : ""}>Price High → Low</option>
                  </select>
                </div>
              </div>
              ...
              <div class="range-box">
                <div class="range-head">
                  <strong style="margin:0;color:#bfdbfe;font-size:11px;letter-spacing:0.35px;text-transform:uppercase;">Price Range</strong>
                  <span>${creationMinPrice}M - ${creationMaxPrice}M</span>
                </div>
                <div class="range-sliders">
                  <div class="range-slider-stack">
                    <div class="range-track"></div>
                    <input type="range" min="${PRICE_MIN_BOUND}" max="${PRICE_MAX_BOUND}" step="1" value="${creationMinPrice}" oninput="setCreationMinPrice(this.value)">
                    <input type="range" min="${PRICE_MIN_BOUND}" max="${PRICE_MAX_BOUND}" step="1" value="${creationMaxPrice}" oninput="setCreationMaxPrice(this.value)">
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="players" id="playersList">
              ${filteredPlayers.length ? filteredPlayers.map(p => {
                ...
                return `
                  <div class="player-row ${isSelected ? "selected" : ""} ${disabled ? "disabled" : ""}" onclick="togglePlayer(${p.id})">
                    <div class="player-left">
                      <div class="player-name">${p.name}</div>
                      <div class="player-meta">
                        <span class="pill">${teamAbbr(p.team)} • ${p.team}</span>
                        <span class="pill">${p.pos} ${countPos(p.pos)}/${squadLimits[p.pos]}</span>
                      </div>
                    </div>
                    <div><strong>${p.price}M</strong></div>
                  </div>
                `;
              }).join("") : `<div class="empty-state">No players match the current filters.</div>`}
            </div>

            <div class="note">
              Rules: exactly 2 GK, 5 DEF, 5 MID, 3 FWD. Maximum ${maxPerTeam} players from one club.
            </div>
          </div>
          ...
          <div class="panel glass">
            <div class="panel-header">
              <div>
                <h3 style="margin-bottom:4px;">My Team</h3>
                <div style="color:#bfdbfe;font-size:13px;">You can also remove players from here.</div>
              </div>
              <div style="font-weight:800;">${selected.length}/15</div>
            </div>

            <div class="squad-groups">
              <div class="pos-group">
                <h4>Goalkeepers ${countPos("GK")}/2</h4>
                ${renderCreationSlots("GK", 2)}
              </div>
              <div class="pos-group">
                <h4>Defenders ${countPos("DEF")}/5</h4>
                ${renderCreationSlots("DEF", 5)}
              </div>
              <div class="pos-group">
                <h4>Midfielders ${countPos("MID")}/5</h4>
                ${renderCreationSlots("MID", 5)}
              </div>
              <div class="pos-group">
                <h4>Forwards ${countPos("FWD")}/3</h4>
                ${renderCreationSlots("FWD", 3)}
              </div>
            </div>

            <div id="creationWarning" class="warning">${getCreationWarning()}</div>
          </div>
        </div>
      ...
      if (playersList) {
        playersList.scrollTop = playersScrollTop;
        playersList.addEventListener("scroll", function () {
          playersScrollTop = this.scrollTop;
        });
      }
    }
    ...
    function renderMain() {
      ...
      const grouped = groupStartersByPos(starterIds);
      const bench = getBenchPlayers();
      const counts = starterCounts(starterIds);
      const viewedTeam = leagueTeams.find(t => t.id === viewedLeagueTeamId) || null;

      const leagueRows = [
        { id: "you", name: teamName, points: totalPoints, isUser: true },
        ...leagueTeams.map(t => ({ id: t.id, name: t.name, points: t.totalPoints, isUser: false }))
      ].sort((a, b) => b.points - a.points);

      document.getElementById("app").innerHTML = `
        <div class="top-bar">
          <h2 style="margin:0;">${teamName}</h2>
          <button class="secondary-btn" onclick="renderCreation()">Transfer Mode</button>
          <button onclick="simulateMatchday()">Simulate Matchday</button>
          ${currentUser ? `<button class="danger-btn" onclick="logoutUser()">Log Out</button>` : ""}
        </div>

        <div class="summary">
          <div class="summary-box glass"><strong>Total Points</strong><span>${totalPoints}</span></div>
          <div class="summary-box glass"><strong>Budget Used</strong><span>${spentBudget()}M</span></div>
          <div class="summary-box glass"><strong>Round</strong><span>${currentRound}</span></div>
          <div class="summary-box glass"><strong>Captain</strong><span>${captain && starterIds.includes(captain) ? getPlayer(captain).name : "None"}</span></div>
          <div class="summary-box glass"><strong>Starters</strong><span>${starterIds.length}/11</span></div>
        </div>

        <div class="controls-box glass">
          <div class="top-bar" style="margin-bottom:10px;">
            <select onchange="setCaptain(this.value)">
              <option value="">Select Captain</option>
              ${starterIds.map(id => {
                const p = getPlayer(id);
                return `<option value="${p.id}" ${captain === p.id ? "selected" : ""}>${p.name} (${p.team})</option>`;
              }).join("")}
            </select>

            <button class="secondary-btn" onclick="clearSwapSelection(); renderMain();">Clear Swap Selection</button>
          </div>

          <div class="helper-line">
            Click a player's card to open full details. Use the Swap button on a starter and then on a bench player to swap them.<br>
            Formation rules: exactly 1 GK, minimum 3 DEF, minimum 2 MID, minimum 1 FWD.<br>
            Current XI: GK ${counts.GK}, DEF ${counts.DEF}, MID ${counts.MID}, FWD ${counts.FWD}.<br>
            Captain gets double points. Bench players do not score.
            ${swapSelection ? `<br><strong>Selected for swap:</strong> ${getPlayer(swapSelection.id).name} (${swapSelection.section})` : ""}
          </div>
        </div>
          ...
          <div class="pitch-wrap">
            ...
            <div class="pitch-layout">
              <div class="pitch-row">
                ${grouped.FWD.map(p => kitCardHTML(p, {
                  clickable: true,
                  selectedSwap: swapSelection && swapSelection.section === "starter" && swapSelection.id === p.id,
                  onClick: `openPlayerModal(${p.id})`,
                  showC: captain === p.id,
                  roundPoints: playerRound(p.id),
                  useSurname: true,
                  actionHtml: `<button class="card-action-btn" onclick="event.stopPropagation(); clickStarterCard(${p.id})">Swap</button>`
                })).join("")}
              </div>
        ...
        <div class="bench-wrap">
          <div class="formation-box glass">
            <h3 style="margin-bottom:10px;">Bench</h3>
            <div class="bench-grid">
              ${bench.map(p => kitCardHTML(p, {
                clickable: true,
                selectedSwap: swapSelection && swapSelection.section === "bench" && swapSelection.id === p.id,
                onClick: `openPlayerModal(${p.id})`,
                showC: captain === p.id,
                roundPoints: playerRound(p.id),
                crossedRound: true,
                useSurname: true,
                actionHtml: `<button class="card-action-btn" onclick="event.stopPropagation(); clickBenchCard(${p.id})">Swap</button>`
              })).join("")}
            </div>
          </div>
        </div>
        ...
        <div class="league-wrap">
          <div class="league-box glass">
            <div class="panel-header">
              <div>
                <h3 style="margin-bottom:4px;">League Table</h3>
                <div style="color:#bfdbfe;font-size:13px;">All teams start at 0. Only starters score after matchday.</div>
              </div>
            </div>

            <div class="league-list">
              ${leagueRows.map((row, index) => `
                <div class="league-team-row ${!row.isUser && row.id === viewedLeagueTeamId ? "active" : ""}">
                  <div><div class="league-team-name">${index + 1}. ${row.name}</div></div>
                  <div class="league-team-points">${row.points} pts</div>
                  <div>${row.isUser ? `<span style="color:#bfdbfe;font-size:13px;">Your team</span>` : `<button class="secondary-btn" onclick="viewLeagueTeam('${row.id}')">${row.id === viewedLeagueTeamId ? "Close Squad" : "View Squad"}</button>`}</div>
                </div>
                ${!row.isUser && row.id === viewedLeagueTeamId && viewedTeam ? renderLeagueTeamViewer(viewedTeam) : ""}
              `).join("")}
            </div>
          </div>
        </div>
